...
public class UserPointController {
	...
	private final UserPointFacade userFacade;
    ...
    public ResponseEntity<UserPointResponse.V1> getBalance(
            @Parameter(description = "유저ID", example = "1")
            @PathVariable("userId") Long userId
    ) {
        UserPoint user = userFacade.getUser(userId);
        return ResponseEntity.ok(UserPointResponse.V1.of(user.id(), user.point()));
    }
    ...
    public ResponseEntity<UserPointResponse.V1> getBalance(
    		@Parameter(description = "유저ID", example = "1")
            @PathVariable("userId") Long userId,
            @RequestBody @Valid UserPointRequest.V1 request
    ) {
	...
	public ResponseEntity<UserPointResponse.V2> getHistory(
			@Parameter(description = "유저ID", example = "1")
            @PathVariable("userId") Long userId
    ) {
		List<UserPointHistory> histories = userFacade.getPointHistories(userId);
        return ResponseEntity.ok(UserPointResponse.V2.of(histories));
    }
}
